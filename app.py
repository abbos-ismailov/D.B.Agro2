...
def detect_disease():
    ...
    if request.method == "POST":
        ...
        classname, advice = detect_class('static/' + file_path)

        with open('data/advice.txt', 'a') as f:
            f.write(classname + f'{hour}:{minutes} {day}-{month}-{year}' + '\n' + advice + '\n' + '\n')
        
        return render_template("detect_disease.html", classname = classname, advice = advice, file_path = file_path, user = user, is_login = is_login, is_admin = is_admin)
...
def sign_up():
    ...
    if request.method == "POST":
        ...
        print(password1, password2, "ASDAHSKJDHAJKSHDKJH")
